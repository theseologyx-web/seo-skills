...
1. Search for business name on each platform:
   ...
   - Apple: manual check (no public API -- recommend Apple Business at business.apple.com — rebranded from Apple Business Connect, March 2026)
...
| Need | Command | Why |
|------|---------|-----|
| Website on-page local signals (NAP, title, H1, schema presence) | `/seo local <url>` | seo-local analyzes the business website; this skill analyzes maps platforms |
| Full AI search visibility (Overviews, ChatGPT, Perplexity) | `/seo geo <url>` | llms.txt, citability scoring, AI Overview appearance |
| Schema validation and JSON-LD fixes | `/seo schema <url>` | Full structured data validation and rich result eligibility |
| Live SERP and keyword data | `/seo dataforseo [command]` | Direct DataForSEO queries outside maps context |
| Brand mention tracking and AI citation signals | `/seo brand <url>` | Brand mentions correlate 3x more with AI visibility than backlinks (Ahrefs 0.664 vs 0.218) |
| Location page content quality | `/seo content <url>` | Unique content %, E-E-A-T signals, thin content — complements maps presence with on-site content depth |
