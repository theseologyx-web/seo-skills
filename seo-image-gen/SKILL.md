...
> **Nota de arquitectura:** Este skill genera assets visuales (OG images, hero images, infographics) — no es análisis SEO estratégico. Se activa puntualmente cuando se necesitan imágenes. Requiere extensión nanobanana-mcp. Para análisis y optimización de imágenes existentes usar seo-images.
...
5. **IPTC metadata (MANDATORY for AI-generated images)**:
   Google requires this on ALL AI-generated images for Google Images, Discover, and Merchant Center:
   ```bash
   # Embed IPTC DigitalSourceType metadata
   exiftool -IPTC:DigitalCreator="TrainedAlgorithmicMedia" output.webp
   # Verify:
   exiftool -IPTC:DigitalCreator output.webp
   ```
   Without this, AI-generated images risk removal from Google surfaces. **Always do this step.**

6. **C2PA metadata (recommended)**: The Coalition for Content Provenance and Authenticity standard is supported by Google, Adobe, Microsoft. Provides a cryptographic manifest declaring AI origin:
   - Adobe Firefly, DALL-E 3, and Midjourney embed C2PA automatically
   - For Gemini-generated images, use Adobe Content Credentials tools or the `c2pa` npm package
   - Not currently mandatory but increasingly expected by platforms

7. **Schema markup**: Suggest `ImageObject` schema for the generated image:
   ```json
   {
     "@type": "ImageObject",
...
8. **OG meta tags**: For social preview images, remind about:
   ```html
...
9. **Google Discover requirements** (for blog hero images):
   - Minimum width: **1,200px** (hard floor) — 1,600px recommended for full card
   - Aspect ratio: **16:9**
   - `max-image-preview:large` meta tag must be present on the page
   - Image must depict actual content (not abstract/decorative)

10. **Google Merchant Center compliance** (for product images):
    - AI-generated product images must be disclosed per Merchant Center policy
    - IPTC metadata above is required
    - Images must show the actual product accurately (no misleading representations)

11. **Visual search optimization** (for product images):
    - Clean white/neutral background for Google Lens recognition
    - Product clearly visible and well-lit
    - If generating lifestyle/in-use shots: ensure product is the primary subject
...
> **Model availability note:** Gemini model names and capabilities change frequently. Before routing to a specific model, verify current availability via Google AI Studio. The names above may be superseded by newer versions.
...
- **seo-content**: images generated here (hero, infographic) complement content E-E-A-T signals — visual assets strengthen experience and expertise perception
- **seo-page**: OG image is audited in single-page analysis (`/seo page`); use `/seo image-gen og` to fix missing or low-quality OG images found there
